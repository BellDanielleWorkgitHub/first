...
describe('isEqualTests', () => {
  ...
  test('если не передать второй аргумент, то будет ошибка выполнения', () => {
    assertThrow(() => isEqual('a'));
  });
  test('если не передать второй аргумент, то будет ошибка теста', () => {
    assertThrow(() => isEqual('a'), 'both parameters are required');
  });
  test('если аргумент null, то будет ошибка теста', () => {
    assertThrow(() => isEqual('a', null), 'both parameters are required');
    assertThrow(() => isEqual(null, null), 'both parameters are required');
    assertThrow(() => isEqual(null, '5'), 'both parameters are required');
  });
